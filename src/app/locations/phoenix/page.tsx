...
export default function Phoenix() {
  useEffect(() => {
    document.title = "HVAC Recruiter Phoenix | Arizona HVAC Technician Recruitment";
    const meta = document.querySelector('meta[name="description"]');
    const content = 'Expert HVAC recruiting in Phoenix, Arizona. Desert climate specialists for high-efficiency cooling, rooftop units, and ductless mini-splits.';
  ...
  const serviceAreas=['Phoenix','Scottsdale','Tempe','Mesa','Chandler','Glendale','Peoria','Gilbert'];
  const specialties=[{title:'EXTREME HEAT COOLING',description:'HVAC systems engineered for 115°F summers and low humidity.',demand:'CRITICAL DEMAND'},{title:'COMMERCIAL ROOFTOP UNITS',description:'Shopping centers & office parks across the Valley.',demand:'HIGH DEMAND'},{title:'DUCTLESS MINI-SPLITS',description:'Retrofit cooling for historic adobe & block homes.',demand:'GROWING DEMAND'},{title:'ENERGY EFFICIENCY',description:'SEER2 20+ installs, smart thermostats, and solar integration.',demand:'STEADY DEMAND'}];

  return (
    <div className="bg-white text-black font-sans">
      <NeuNavbar navItems={navItems}/>
      <section className="pt-32 pb-20 px-4 bg-gray-100 text-center">
        <div className="max-w-6xl mx-auto">
          <motion.h1 className="text-4xl md:text-6xl lg:text-7xl font-black uppercase tracking-wider mb-8" initial={{scale:0.5,opacity:0}} animate={{scale:1,opacity:1}} transition={{duration:1}}>HVAC RECRUITER <span className="text-purple-500">PHOENIX</span></motion.h1>
          <motion.p className="text-lg md:text-xl lg:text-2xl font-bold mb-12 max-w-4xl mx-auto" initial={{x:-100,opacity:0}} animate={{x:0,opacity:1}} transition={{duration:0.8,delay:0.3}}>I track down desert-tested HVAC talent so your business can keep Arizona cool.</motion.p>
          <NeuButton onClick={()=>window.location.href='mailto:yeshaya@hvacrecruiter.co?subject=Phoenix HVAC Recruiting Inquiry'} variant="primary" size="lg">START HUNTING PHOENIX TECHS</NeuButton>
        </div>
      </section>
      <section className="py-20 px-4 bg-white"><div className="max-w-6xl mx-auto">
        <motion.h2 className="text-4xl md:text-6xl font-black uppercase text-center mb-16" initial={{opacity:0,y:50}} whileInView={{opacity:1,y:0}} transition={{duration:0.8}} viewport={{once:true}}>PHOENIX <span className="text-purple-500">SPECIALTIES</span></motion.h2>
